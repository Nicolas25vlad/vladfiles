...
BarWidget {
  ...
  // A little more breathing room keeps the title readable inside the shared
  // right-side capsule without letting one track swallow the status bar.
  ...
  Row {
    ...
    Text {
      ...
      text: root.playIcon
      color: activePlayer && activePlayer.isPlaying ? Color.accent : Qt.darker(root.bar.barForeground, 1.5)
    ...
    Item {
      ...
      implicitWidth: Math.min(root.maxLabelWidth, labelText.implicitWidth)
      height: glyph.height
      clip: true
      anchors.verticalCenter: parent.verticalCenter
      ...
      Text {
        ...
        font.family: root.bar.fontFamily
        font.pixelSize: Style.font.body
        font.bold: activePlayer && activePlayer.isPlaying
        anchors.verticalCenter: parent.verticalCenter
