...
Item {
  ...
  readonly property real barCornerRadius: configuredCornerRadius >= 0 ? configuredCornerRadius : 0
  ...
  component BarPanel: PanelWindow {
    ...
    // Keep the layer-surface transparent: the three bar sections below draw
    // their own backgrounds, leaving the gaps between sections untouched.
    color: "transparent"
  ...
  component SectionChrome: BorderSurface {
    id: sectionChrome

    required property var entries
    required property string region
    readonly property real horizontalPadding: Style.space(8)
    readonly property real verticalPadding: Style.space(4)

    visible: entries.length > 0
    color: root.transparent
      ? "transparent"
      : Qt.rgba(root.background.r, root.background.g, root.background.b, 0.98)
    borderSpec: Border.controlSpec("normal", root.barForeground, Color.accent, root.urgent)
    radius: Math.min(Style.space(9), height / 2)
    implicitWidth: visible ? sectionModules.width + horizontalPadding : 0
    implicitHeight: visible ? sectionModules.height + verticalPadding : 0
    width: implicitWidth
    height: implicitHeight

    ModuleList {
      id: sectionModules
      entries: sectionChrome.entries
      region: sectionChrome.region
      anchors.centerIn: parent
    }
  }

  component LeftModules: SectionChrome {
    entries: root.layoutEntries("left")
    region: "left"
  }

  component RightModules: SectionChrome {
  ...
  component CenterModules: Item {
    ...
    Component {
      ...
      Item {
        ...
        readonly property real chromePadding: Style.space(4)
        readonly property real centerContentWidth: centerBefore.width + centerAnchorModule.width + centerAfter.width

        BorderSurface {
          id: centerChrome
          z: -1
          x: centerRoot.hasAnchor
            ? centerAnchorModule.x - centerBefore.width - chromePadding
            : centerPlain.width > 0 ? centerPlain.x - chromePadding : 0
          y: centerRoot.hasAnchor
            ? centerAnchorModule.y - chromePadding
            : centerPlain.height > 0 ? centerPlain.y - chromePadding : 0
          width: centerRoot.hasAnchor
            ? centerBefore.width + centerAnchorModule.width + centerAfter.width + chromePadding * 2
            : centerPlain.width + chromePadding * 2
          height: centerRoot.hasAnchor
            ? Math.max(centerBefore.height, centerAnchorModule.height, centerAfter.height) + chromePadding * 2
            : centerPlain.height + chromePadding * 2
          color: root.transparent
            ? "transparent"
            : Qt.rgba(root.background.r, root.background.g, root.background.b, 0.98)
          borderSpec: Border.controlSpec("normal", root.barForeground, Color.accent, root.urgent)
        ...
        ModuleSlot {
          ...
          region: "center"
          anchors.verticalCenter: parent.verticalCenter
          x: (parent.width - centerContentWidth) / 2 + centerBefore.width
        }
    ...
    Component {
      ...
      Item {
        ...
        readonly property real chromePadding: Style.space(4)
        readonly property real centerContentHeight: centerBeforeVertical.height + centerAnchorModule.height + centerAfterVertical.height

        BorderSurface {
          id: centerChromeVertical
          z: -1
          x: centerRoot.hasAnchor
            ? centerAnchorModule.x - chromePadding
            : centerPlainVertical.width > 0 ? centerPlainVertical.x - chromePadding : 0
          y: centerRoot.hasAnchor
            ? centerAnchorModule.y - centerBeforeVertical.height - chromePadding
            : centerPlainVertical.height > 0 ? centerPlainVertical.y - chromePadding : 0
          width: centerRoot.hasAnchor
            ? Math.max(centerBeforeVertical.width, centerAnchorModule.width, centerAfterVertical.width) + chromePadding * 2
            : centerPlainVertical.width + chromePadding * 2
          height: centerRoot.hasAnchor
            ? centerBeforeVertical.height + centerAnchorModule.height + centerAfterVertical.height + chromePadding * 2
            : centerPlainVertical.height + chromePadding * 2
          color: root.transparent
            ? "transparent"
            : Qt.rgba(root.background.r, root.background.g, root.background.b, 0.98)
          borderSpec: Border.controlSpec("normal", root.barForeground, Color.accent, root.urgent)
          radius: Math.min(Style.space(9), width / 2)
        }

        ModuleList {
          id: centerPlainVertical
          visible: !centerRoot.hasAnchor
          entries: centerRoot.entries
          region: "center"
          anchors.centerIn: parent
        ...
        ModuleSlot {
          ...
          region: "center"
          anchors.horizontalCenter: parent.horizontalCenter
          y: (parent.height - centerContentHeight) / 2 + centerBeforeVertical.height
        }

        ModuleList {
          id: centerAfterVertical
          visible: centerRoot.hasAnchor
          entries: root.entriesAfter(centerRoot.entries, root.centerAnchor)
          region: "center"
  ...
  component ModuleSlot: Item {
    ...
    function injectProps() {
      ...
      // First-party widgets normally declare their canonical id themselves;
      // assigning it again can hit a read-only binding during Loader reloads.
      if (!commandCustom && "moduleName" in target && !target.moduleName) target.moduleName = moduleName
